...
class LoginViewModel @Inject constructor(private val repository: AuthRepository) : ViewModel() {
    ...
    fun login(email: String, senha: String, ativo : Int = 1) { // inicia o processo de Login, chamada pela UI
        viewModelScope.launch { //Corrotina --> Executar código assincrono, sem travar interface, se não tivesse iria travar a tela
            ...
            try {
                val response = repository.login(LoginModelRequest(ativo,email,senha))
    ...
    fun resetLoginState() {
        ...
        _loginResult.value = Result.Loading // ou qualquer outro estado que faça sentido
    }
